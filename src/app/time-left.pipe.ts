...
export class TimeLeftPipe implements PipeTransform {
    transform(dueDate: Date): string {
        ...
        const formattedTime = this.formatTimeUnits(timeUnits);
        return `${formattedTime} left`;
    }

    private calculateSecondsDifference(
        dueDate: Date,
        currentDate: Date
    ): number {
    ...
    private calculateTimeUnits(secondsLeft: number): {
        days: number;
        hours: number;
        minutes: number;
    } {
    ...
    private formatTimeUnits(timeUnits: {
        days: number;
        hours: number;
        minutes: number;
    }): string {
        ...
        if (timeUnits.hours > 0) {
            result += `${timeUnits.hours} hour${
                timeUnits.hours > 1 ? "s" : ""
            } `;
        }
        if (timeUnits.minutes > 0) {
            result += `${timeUnits.minutes} minute${
                timeUnits.minutes > 1 ? "s" : ""
            } `;
        }
        return result.trim();
    }
